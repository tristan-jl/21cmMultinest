...
nopeaks = 2
mu = np.random.uniform(0., 10., nopeaks)
sigma = np.random.uniform(0.2, 2.0, nopeaks)

print("mu=", mu, "width=", sigma)

x=np.linspace(0., 10., 11)
data=sp.stats.norm.pdf(x, mu[0], 1.) + sp.stats.norm.pdf(x, mu[1], 1.)
noise=0.1

def Model(centre, width):
    return sp.stats.norm.pdf(x, centre, width)
...
# run MultiNest
pymultinest.run(Loglike, Prior, n_params, importance_nested_sampling = False, resume = True, verbose = True, sampling_efficiency = 'model', n_live_points = 5000, outputfiles_basename='chains/2-')
