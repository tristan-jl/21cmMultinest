...
class MN2:
    ...
    def generate_data(self, filename, noise=0.03):
        ...
        Args:
            ...
            noise (float)
        """
        x0, y0, z0 = np.random.uniform(*self.x_range, size=self.num), np.random.uniform(*self.x_range, size=self.num), np.random.uniform(*self.x_range, size=self.num)
        width = np.random.uniform(0.5, 3., self.num)
        amp = np.random.uniform(0.1, 1., self.num)
