...
#data=Gaussian_2D(np.array([xx, yy]), mu[0], 1.)

for i in range(len(mu)):
    data+=Gaussian_2D(np.array([xx, yy]), mu[i], sigma[i])

noise=0.1

def Model(centre, width):
    return Gaussian_2D(np.array([xx, yy]), centre, width)
...
def Loglike(cube, ndim, nparams):
    ...
    model=Model(np.array([cube[0], cube[1]]), 1.)
...
# run MultiNest
pymultinest.run(Loglike, Prior, n_params, importance_nested_sampling = False, resume = False, verbose = True, sampling_efficiency = 'model', n_live_points = 1000, outputfiles_basename='chains/')
